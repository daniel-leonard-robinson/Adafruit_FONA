...
//
//	uint16_t len = 0;
//	for (uint16_t i=0; i<packets_len; i++) {
...
////	if (replybuffer[0] != '>') return false;
//
//	for (uint16_t j=0; j<packets_len; j++) {
//		mySerial->write(packets[j], packets_len);
//	}
//
//	readline(3000); // wait up to 3 seconds to send the data
//
...
//boolean Adafruit_FONA::TCPsend(String packet, uint8_t len) {
//
//  DEBUG_PRINT(F("AT+CIPSEND="));
//  DEBUG_PRINTLN(len);
//#ifdef ADAFRUIT_FONA_DEBUG
